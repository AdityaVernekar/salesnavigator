...
type RunPipelineFromContactsDialogProps = {
  open: boolean;
...
export function RunPipelineFromContactsDialog({
  open,
  onOpenChange,
  contactCount,
  isSubmitting,
  onSubmit,
}: RunPipelineFromContactsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Run pipeline on {contactCount} contact{contactCount !== 1 ? "s" : ""}
          </DialogTitle>
          <DialogDescription>
            Choose which stages to run. Only the selected contacts will be
            processed.
          </DialogDescription>
        </DialogHeader>
