  ...
  try {
    ...
    if (contactsError || !contacts?.length) {
      return NextResponse.json(
        { ok: false, error: "Contacts not found or inaccessible." },
        { status: 404 },
      );
    }
    ...
    if (contacts.length !== parsed.contactIds.length) {
      return NextResponse.json(
        { ok: false, error: "Some selected contacts are missing." },
        { status: 400 },
      );
    }

    const leadIds = Array.from(new Set(contacts.map((contact) => contact.lead_id).filter(Boolean)));
    if (!leadIds.length) {
      return NextResponse.json(
        { ok: false, error: "Selected contacts are not linked to leads." },
        { status: 400 },
      );
    }
    ...
    if (leadsError || !leads?.length) {
      return NextResponse.json(
        { ok: false, error: "Linked leads not found or inaccessible." },
        { status: 404 },
      );
    }
    ...
    if (campaignIds.size > 1) {
      return NextResponse.json(
        { ok: false, error: "All selected contacts must belong to the same campaign." },
        { status: 400 },
      );
    }
    ...
    if (!campaignId) {
      return NextResponse.json(
        { ok: false, error: "Selected contacts have no campaign." },
        { status: 400 },
      );
    }
    ...
    await logRunEvent(run.id, "pipeline", "info", "Pipeline run queued (from selected contacts)", {
      campaignId,
      contactCount: parsed.contactIds.length,
    ...
    if (runId) {
      ...
      await logRunEvent(runId, "pipeline", "error", "Pipeline enqueue failed (from contacts)", {
        error: errorMessage,
      });
    }
    return NextResponse.json(
